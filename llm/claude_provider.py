...
class ClaudeProvider:
    ...
    def _options(self, *, system: str = "", schema: Optional[dict] = None, **over):
        ...
        if not self.allowed_tools:
            ...
            #   허용이 먹지 않았다. 쇼츠의 "키워드 → 자동 리서치" 를 켜는 이음새가
            #   정확히 이 세 줄이다. 허용한 것만 차단 목록에서 뺀다.
            keep = {t.strip() for t in self.allowed_tools if t and t.strip()}
            disallowed = [d for d in disallowed if d not in keep]
