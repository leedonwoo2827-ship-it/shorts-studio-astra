...
def dict_path() -> Optional[Path]:
    ...
    ★ **레포 안 사전이 먼저다.** 이 앱은 다른 폴더에 기대지 않는다 —
      노트북에 레포만 옮겨도 발음이 그대로 따라와야 한다.
      voicewright 를 붙인 사람은 그쪽 사전도 쓸 수 있게 뒤에 남겨 둔다.
    """
    from core import config

    local = Path(__file__).resolve().parents[1] / "config_data" / "pronunciation_map.yaml"
    if local.is_file():
        return local
